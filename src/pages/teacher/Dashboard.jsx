...
import { demoStudents, demoSubjects, demoAnnouncements } from '../../data/demoData';

const TeacherDashboard = () => {
 const { user } = useAuth();

 const mySubjects = demoSubjects.filter(s => s.teacherId === 'demo-teacher');
 const studentsInClass = demoStudents.filter(s => s.section === 'A');
 const avgCgpa = (studentsInClass.reduce((sum, s) => sum + s.cgpa, 0) / studentsInClass.length).toFixed(1);

 const barData = mySubjects.map(s => ({
  subject: s.name.length > 10 ? s.name.slice(0, 10) + '…' : s.name,
  students: studentsInClass.filter(st => st.enrolledSubjects.includes(s.id)).length || Math.floor(Math.random() * 15) + 25,
 ...
 return (
  <div className="animate-fade-in" style={{ padding: '24px 28px' }}>
   ...
   <div style={{
    ...
    <StatCard title="Total Students" value={studentsInClass.length} subtitle="Section A" icon={HiOutlineUserGroup} color="#9B8EC7" />
    ...
    <StatCard title="Subjects Assigned" value={mySubjects.length} subtitle="Current semester" icon={HiOutlineBookOpen} color="#7EC8A0" />
   ...
   <div className="dashboard-grid-2col">
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
     <div className="glass-card" style={{ padding: '18px 22px' }}>
      ...
      {mySubjects.map((s) => (
       <div key={s.id} style={{
        ...
        alignItems: 'center',
        padding: '8px 0',
        borderBottom: '1px solid rgba(var(--border-rgb), 0.15)',
       }}>
        <div>
     ...
     <div className="glass-card" style={{ padding: '18px 22px' }}>
      ...
      {demoAnnouncements.slice(0, 2).map((a) => (
       <div key={a.id} style={{
        padding: '8px 0',
        borderBottom: '1px solid rgba(var(--border-rgb), 0.15)',
       }}>
