...
import { demoStudents, demoTeachers } from '../../data/demoData';

const AdminDashboard = () => {
 const deptData = [
  { name: 'CSE', students: 35, attendance: 84 },
 ...
 const pieData = [
  { name: 'CSE', value: 35, color: '#9B8EC7' },
  { name: 'ECE', value: 28, color: '#B4D3D9' },
  { name: 'EEE', value: 22, color: '#7EC8A0' },
  { name: 'ME', value: 30, color: '#BDA6CE' },
 ];

 const feeCollected = demoStudents.filter(s => s.feeStatus === 'paid').length * 120000;
 const feePending = demoStudents.filter(s => s.feeStatus!== 'paid').reduce((sum, s) => sum + s.feeDue, 0);
 ...
 return (
  <div className="animate-fade-in" style={{ padding: '24px 28px' }}>
   ...
   <div style={{
    ...
    <StatCard title="Total Students" value={demoStudents.length} subtitle="Across departments" icon={HiOutlineUserGroup} color="#9B8EC7" trend="+12 this sem" trendUp />
    <StatCard title="Total Teachers" value={demoTeachers.length} icon={HiOutlineUsers} color="#B4D3D9" />
    <StatCard title="Avg Attendance" value="81%" subtitle="College-wide" icon={HiOutlineChartBarSquare} color="#7EC8A0" trend="+3.2%" trendUp />
    <StatCard title="Fee Collected" value={`₹${(feeCollected / 100000).toFixed(1)}L`} subtitle={`₹${(feePending / 1000).toFixed(0)}K pending`} icon={HiOutlineCurrencyRupee} color="#BDA6CE" />
   </div>
     ...
     <div style={{ height: '280px' }}>
      <ResponsiveContainer width="100%" height="100%">
       <BarChart data={deptData}>
        ...
        <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ background: 'var(--bg-card)', border: '1px solid rgba(var(--border-rgb), 0.4)', borderRadius: '8px', fontSize: '0.8rem' }} />
